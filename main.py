...
async def on_message(message):  #메시지에 반응하는 함수 (매개변수는 message)
    ...
    #출석
    if message.content.startswith("!출석조회"):
        parts = message.content.split()
        if len(parts) == 1: user = message.author.id
        else: user = parts[1]
        await message.channel.send(AttDB.attendance_check(user))
    elif message.content.startswith("!출석순위"):
        await message.channel.send(AttDB.attendance_rank(message.author.id))
    elif message.content.startswith("!출석"):
        await message.channel.send(AttDB.attendance(message.author.id, message.author.display_name))
